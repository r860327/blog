...
BPF可以将filter放到kernel space，这样可以减少user space的cpu负载。使用该功能需要包含头文件`#include <linux/filter.h>`
...
关于tcpdump中的过滤命令可以参考[1](http://www.tcpdump.org/manpages/pcap-filter.7.html)。
...
具体每个指令的含义可以参考[2](https://www.kernel.org/doc/Documentation/networking/filter.txt)。
